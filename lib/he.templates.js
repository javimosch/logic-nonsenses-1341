...
function* buildTemplatesPartials(src, append) {
    return new Promise(function(resolve, err) {
        co(function*() {
            ...
            let partials = heUtils.normalizeFilesTreePreservePath(readDirFiles.readSync(src || PATH));
            partials = heUtils.filesIncludeOnly(partials, PARTIALS_EXT);
            let keys = Object.keys(partials),
                key = null,
            ...
            for (var x in keys) {
               ...
               // }
                ...
                if (isAfter) {
                    ...
                    heFirebase.sendPartial(name, partials[key], fullPath); //async
                    Handlebars.registerPartial(name, partials[key]); //We finally register the handlebar partial
                }
    ...
    return new Promise((resolve, error) => {
        ...
        function handleNewFileTransform(raw, path) {
            ...
            if (needsCompilation(raw, path)) {
                ...
                //console.log('DEBUG: skip compilation (static) ',path);
            }
            return rta;
        }
