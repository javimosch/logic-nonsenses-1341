...
if (!config.deploy) {
    console.log('check deploy section', APP_NAME);
    process.exit(0);
}

if (!config.deploy.ftp) {
    console.log('check deploy.ftp section', APP_NAME);
    process.exit(0);
}


if (!auth.hostname || !auth.port || !auth.username || !auth.password) {
    console.log('check authentification for', APP_NAME, 'auth', JSON.stringify(auth));
    process.exit(0);
}
